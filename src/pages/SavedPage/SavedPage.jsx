  ...
  const gotIt = useMemo(() => {
    if (
      ...
      if (savedTasks[details.id].responces) {
        // if (savedTasks[details.id].responces.find(e => e.user.id === "window.Telegram.WebApp.initDataUnsafe.user.last_name.id")){
        //   return true
        // }
        // else{
        //   return false
    ...
    if (responce.isOpen && !responce.shablonMaker) {
      window.Telegram.WebApp.showPopup(
        ...
        (buttonId) => {
          ...
          if (buttonId === "save") {
            postResponce(savedTasks[details.id].id, window.Telegram.WebApp.initDataUnsafe.user.last_name.id);
            setResponce((value) => ({ ...value, isOpen: false }));
            setDetails((value) => ({ ...value, isOpen: false }));
          }
        }
